...
async function getData(id: number) {
  const res = await fetch(`https://jsonplaceholder.typicode.com/todos/${id}`, {
    cache: "no-store", // Avoids caching API response
  })
...
export default async function ServerPage() {
  const randomId = Math.floor(Math.random() * 10) + 1 // Random ID for testing
      ...
      <Card className="w-full max-w-md shadow-lg">
        <CardHeader className="bg-gradient-to-r from-blue-500 to-purple-600 text-white">
          <div className="flex items-center justify-between">
            <CardTitle className="text-xl font-bold">Server-Side Rendering (SSR)</CardTitle>
            <Badge variant="outline" className="border-white text-white">
              Task #{data.id}
            </Badge>
          </div>
          <CardDescription className="text-blue-100">Dynamically rendered on each request</CardDescription>
        </CardHeader>
